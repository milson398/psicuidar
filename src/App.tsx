...
const App: React.FC = () => {
  ...
  useEffect(() => {
    if (token && res) {
      const handleTokenResponse = async () => {
        ...
        try {
          await supabase.from('appointments').update({ status: newStatus, is_viewed: false }).eq('confirmation_token', token);
    ...
    return (
      <div className="min-h-screen bg-[#060b18] flex items-center justify-center p-4">
        <div className="bg-[#111827] p-10 rounded-3xl shadow-2xl max-w-md w-full text-center border border-white/5">
          ...
          <h2 className="text-3xl font-black text-white mb-4">Recebido!</h2>
          <p className="text-gray-400 text-lg font-medium leading-relaxed mb-10">{studentResponse.msg}</p>
          <div className="pt-6 border-t border-white/5 text-[10px] text-gray-600 font-bold tracking-[0.2em] uppercase">PsiCuidar Sistema Seguro</div>
        </div>
      </div>
    );
  }
  ...
  if (currentView === 'LOADING_TOKEN') {
    return <div className="min-h-screen bg-[#060b18] flex items-center justify-center"><div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-blue-500"></div></div>;
  }
  ...
  const renderPage = () => {
    const commonProps = { onUpdateStatus: (id: string, s: AppointmentStatus) => supabase.from('appointments').update({ status: s, is_viewed: true }).eq('id', id).then(() => fetchAppointments()) };
    switch (activePage) {
      case 'Dashboard': return <Dashboard appointments={appointments} onUpdateStatus={commonProps.onUpdateStatus} userName={userName} />;
      ...
      default: return <Dashboard appointments={appointments} onUpdateStatus={commonProps.onUpdateStatus} userName={userName} />;
    }
  };

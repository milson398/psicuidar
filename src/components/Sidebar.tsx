...
const NavItem: React.FC<{ icon: React.ReactNode; label: string; isActive: boolean; onClick: () => void; activeClass: string; textClass: string }> = ({ icon, label, isActive, onClick, activeClass, textClass }) => (
  <li className="my-0.5">
    <a
      ...
      className={`flex items-center p-2 rounded-lg transition-all duration-300 ${isActive
    ...
    >
      ...
      <span className={`ml-3 text-sm font-medium ${isActive ? 'text-white' : 'text-inherit'}`}>{label}</span>
    </a>
  </li>
);

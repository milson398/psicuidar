...
const Sidebar: React.FC<SidebarProps> = ({ setActivePage, activePage, themeColor, isOpen, onClose, userEmail, isFuncionario, onLogout }) => {
  ...
  return (
    <>
      {isOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-20 md:hidden backdrop-blur-sm transition-opacity" onClick={onClose}></div>
      )}
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 shadow-xl transform transition-transform duration-300 ease-in-out md:relative md:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'} ${containerClass} flex flex-col h-screen`}>
        <div className="h-16 pt-safe-area px-4 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center flex-shrink-0">
          ...
          <button onClick={onClose} className="md:hidden mt-safe-area text-gray-500 hover:text-gray-700 focus:outline-none"><svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg></button>
        </div>
